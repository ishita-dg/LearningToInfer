...
for (m in c(301:340)){
  modeljson<-fromJSON(file = paste0("nhid3_part",m,"_", type, "preds_disc_epoch30_sg0_f100.1_Nb100_Nt1.json"))
...
# dm <- dm[dm$accuracy_ratio < 4,]
# dm <- dm[dm$accuracy_ratio > 0,]
# dm <- dm[(dm$frational-dm$prior)/(dm$fpeople-dm$prior) > 0,]
dm$rational <- abs(dm$frational - dm$prior)
dm$people <- abs(dm$fpeople - dm$prior)

dm <- dm[dm$rational > 0.0005,]

# # bin by rational update
# N = 4
# bins = quantile(dm$rational, probs=0:N/N)
# dm <- within(dm, group <- as.integer(cut(rational, bins, include.lowest=TRUE)))
# 
